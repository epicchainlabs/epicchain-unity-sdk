...
namespace EpicChain.Unity.SDK.Tests.Transaction
{
    /// <summary>
    ...
    public class TransactionBuilderTests
    {
        ...
        public async Task TestBuildTransactionWithCorrectNonce()
        {
            ...
            var transactionBuilder = new TransactionBuilder(mockEpicChainSwift)
                .ValidUntilBlock(1)
        ...
        public void TestFailBuildingTransactionWithInvalidBlockNumber()
        {
            ...
            Assert.Throws<ArgumentOutOfRangeException>(() =>
            {
                new TransactionBuilder(mockEpicChainSwift)
                    .ValidUntilBlock(-1)
                    .Script(new byte[] { 1, 2, 3 })
                    .Signers(new AccountSigner[] { AccountSigner.CalledByEntry(account1) });
            });
        ...
        public async Task TestFailBuildingTxWithoutAnySigner()
        {
            ...
            {
                await new TransactionBuilder(mockEpicChainSwift)
                    .ValidUntilBlock(100)
                    .Script(new byte[] { 1, 2, 3 })
                    .GetUnsignedTransactionAsync();
            });
        ...
        public void TestFailAddingMoreThanMaxAttributesToTx_JustAttributes()
        {
            ...
            Assert.Throws<ArgumentException>(() =>
            {
                new TransactionBuilder(mockEpicChainSwift).Attributes(attrs);
            });
        }
        ...
        public async Task TestAutomaticSettingOfValidUntilBlockVariable()
        {
            ...
            // Assert
            var expectedValidUntil = EpicChainSwiftDEFAULT_MAX_VALID_UNTIL_BLOCK_INCREMENT + 999;
        ...
        public void TestExtendScript()
        {
            ...
            var builder = new TransactionBuilder(mockEpicChainSwift).Script(script1);
        ...
        public IEnumerator TestUnityCoroutineCompatibility()
        {
            ...
            EpicChain.Unity.SDK.Models.Transaction result = null;
        ...
        public void TestEdgeCases_InvalidInputs()
        {
            ...
            {
                new TransactionBuilder(mockEpicChainSwift).Script(null);
            });

            // Test empty script
            Assert.Throws<ArgumentException>(() =>
            {
                new TransactionBuilder(mockEpicChainSwift).Script(new byte[0]);
            });

            // Test null signers
            Assert.Throws<ArgumentNullException>(() =>
            {
                new TransactionBuilder(mockEpicChainSwift).Signers(null);
            });

            // Test empty signers array
            Assert.Throws<ArgumentException>(() =>
            {
                new TransactionBuilder(mockEpicChainSwift).Signers(new AccountSigner[0]);
            });
        }
        ...
        public async Task TestTransactionSerialization()
        {
            ...
            var deserializedTx = EpicChain.Unity.SDK.Models.Transaction.FromByteArray(serializedTx);
