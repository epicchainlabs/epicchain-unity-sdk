...
namespace EpicChain.Unity.SDK.Tests.Wallet
{
    ...
    public class AccountTests
    {
        ...
        #region XEP-6 Conversion Tests
        
        [Test]
        public async Task TestToXEP6Account_WithEncryption_ShouldCreateValidXEP6()
        {
            // Arrange
            var account = new Account(testKeyPair);
            var password = "test_password";
            ...
            // Act
            var xep6Account = await account.ToXEP6Account();
            
            // Assert
            Assert.IsNotNull(xep6Account, "XEP-6 account should not be null");
            Assert.AreEqual(testAddress, xep6Account.Address, "Address should match");
            Assert.IsNotNull(xep6Account.Key, "Encrypted key should not be null");
            Assert.IsNotNull(xep6Account.Contract, "Contract should not be null");
        }
        
        [Test]
        public async Task TestFromXEP6Account_ShouldCreateValidAccount()
        {
            // Arrange
            var originalAccount = new Account(testKeyPair);
            var password = "test_password";
            await originalAccount.EncryptPrivateKey(password);
            var xep6Account = await originalAccount.ToXEP6Account();
            
            // Act
            var restoredAccount = await Account.FromXEP6Account(xep6Account);
