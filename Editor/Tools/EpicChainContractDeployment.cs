...
namespace EpicChain.Unity.SDK.Editor.Tools
{
    ...
    public class NeoContractDeployment : EditorWindow
    {
        ...
        private EpicChainWallet deploymentWallet;
        private string walletPassword = "";
        
        // Network configuration
        private EpicChainUnityConfig deploymentConfig;
        ...
        private void DrawNetworkConfiguration()
        {
            ...
            deploymentConfig = EditorGUILayout.ObjectField("Network Config:", deploymentConfig, typeof(EpicChainUnityConfig), false) as EpicChainUnityConfig;
        ...
        private async Task TestNetworkConnection()
        {
            try
            {
                var success = await EpicChainUnityInstance.Initialize(deploymentConfig);
        ...
        private async Task CreateDeploymentWallet()
        {
            try
            {
                deploymentWallet = await EpicChainWallet.Create();
        ...
        private async Task LoadWalletFromFile(string filePath)
        {
            try
            {
                deploymentWallet = await EpicChainWallet.FromXEP6WalletFile(filePath);
        ...
        private async Task DeployContract()
        {
            try
            {
                ...
                var transaction = new TransactionBuilder(EpicChainUnityInstance)
                ...
                var response = await EpicChainUnityInstance.SendRawTransaction(signedTransaction.ToHexString()).SendAsync();
