...
namespace EpicChain.Unity.SDK.Editor.Tools
{
    ...
    public class EpicChainUnityWindow : EditorWindow
    {
        ...
        // Network tab fields
        private EpicChainUnityConfig networkConfig;
        ...
        private void OnGUI()
        {
            ...
            // Header
            EditorGUILayout.LabelField("EpicChain Unity SDK Developer Tools", EditorStyles.boldLabel);
        ...
        private void DrawWalletTab()
        {
            ...
            {
                var path = EditorUtility.OpenFilePanel("Select XEP-6 Wallet", "", "json");
        ...
        private async Task CreateNewWallet()
        {
            try
            {
                ...
                {
                    currentWallet = await EpicChainWallet.Create(walletPassword);
                }
        ...
        private async Task LoadWallet()
        {
            try
            {
                currentWallet = await EpicChainWallet.FromXEP6WalletFile(walletFilePath);
        ...
        private async Task SaveCurrentWallet()
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(path))
                {
                    await currentWallet.SaveXEP6Wallet(path);
        ...
        private async Task CallContractFunction()
        {
            try
            {
                if (!EpicChainUnityInstance.IsInitialized)
                {
                    await EpicChainUnityInstance.Initialize();
                }
                
                var hash = new Hash160(contractHash);
                var response = await EpicChainUnityInstance.InvokeFunction(hash, contractFunction, new System.Collections.Generic.List<Types.ContractParameter>()).SendAsync();
        ...
        private async Task GetContractInfo()
        {
            try
            {
                ...
                var hash = new Hash160(contractHash);
                var response = await EpicChainUnityInstance.GetContractState(hash).SendAsync();
        ...
        {
            var config = ScriptableObject.CreateInstance<EpicChainUnityConfig>();
        ...
        private async Task TestNetworkConnection()
        {
            try
            {
                var config = networkConfig ?? EpicChainUnityConfig.CreateTestnetConfig();
                var success = await EpicChainUnityInstance.Initialize(config);
                
                if (success)
                {
                    var version = await EpicChainUnityInstance.GetVersion().SendAsync();
        ...
        private async Task GetNetworkInfo()
        {
            ...
            {
                var config = networkConfig ?? EpicChainUnityConfig.CreateTestnetConfig();
                await EpicChainUnityInstance.Initialize(config);
                
                var versionTask = EpicChainUnityInstance.GetVersion().SendAsync();
                var blockCountTask = EpicChainUnityInstance.GetBlockCount().SendAsync();
                var peersTask = EpicChainUnityInstance.GetPeers().SendAsync();
        ...
        private void DrawToolsTab()
        {
            ...
            {
                Application.OpenURL("https://epic-chain.org/");
            }
            
            if (GUILayout.Button("Open GitHub Repository"))
            {
                Application.OpenURL("https://github.com/epicchainlabs/epicchain-unity-sdk");
            }
        }
        ...
        {
            var isInitialized = EpicChainUnityInstance.IsInitialized;
            var config = EpicChainUnityInstance.Config;
            
            var status = $"EpicChain Unity SDK Status:\n\n" +
