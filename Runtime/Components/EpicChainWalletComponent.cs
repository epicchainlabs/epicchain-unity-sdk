...
namespace EpicChain.Unity.SDK.Components
{
    ...
    /// </summary>
    public class EpicChainWalletComponent : MonoBehaviour
    {
        ...
        public event Action<EpicChainWallet> OnWalletLoaded;
        ...
        public async Task<EpicChainWallet> CreateNewWallet(string walletName = null)
        {
            try
            {
                wallet = await EpicChainWallet.Create();
                ...
                if (showWalletInfo)
                {
                    Debug.Log($"[EpicChainWalletComponent] Created new wallet: {wallet.Name}");
                    Debug.Log($"[EpicChainWalletComponent] Default address: {wallet.DefaultAccount.Address}");
                }
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[EpicChainWalletComponent] {errorMessage}");
        ...
        public async Task<EpicChainWallet> LoadWallet(string filePath)
        {
            ...
            try
            {
                wallet = await EpicChainWallet.FromXEP6WalletFile(filePath);
                ...
                if (showWalletInfo)
                {
                    Debug.Log($"[EpicChainWalletComponent] Loaded wallet: {wallet.Name} from {filePath}");
                    Debug.Log($"[EpicChainWalletComponent] Accounts: {wallet.AccountCount}");
                }
        ...
        /// <summary>
        /// Saves the current wallet to a XEP-6 file.
        /// </summary>
        ...
        public async Task SaveWallet(string filePath = null)
        {
            ...
            try
            {
                await wallet.SaveXEP6Wallet(saveFilePath);
                walletFilePath = saveFilePath;
                
                if (showWalletInfo)
                {
                    Debug.Log($"[EpicChainWalletComponent] Saved wallet to: {saveFilePath}");
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"[EpicChainWalletComponent] Failed to save wallet: {ex.Message}");
                throw;
            }
        }
        ...
        public async Task<Account> CreateNewAccount()
        {
            ...
            try
            {
                ...
                if (showWalletInfo)
                {
                    Debug.Log($"[EpicChainWalletComponent] Created new account: {account.Address}");
                }
            ...
            {
                Debug.LogError($"[EpicChainWalletComponent] Failed to create account: {ex.Message}");
                throw;
            }
        }
        ...
        public void LockWallet()
        {
            if (wallet != null)
            {
                ...
                if (showWalletInfo)
                {
                    Debug.Log("[EpicChainWalletComponent] Wallet locked");
                }
        ...
        public async Task UnlockWallet(string password = null)
        {
            ...
            try
            {
                ...
                if (showWalletInfo)
                {
                    Debug.Log("[EpicChainWalletComponent] Wallet unlocked");
                }
            ...
            {
                Debug.LogError($"[EpicChainWalletComponent] Failed to unlock wallet: {ex.Message}");
                throw;
            }
        }
        ...
        private void CheckAutoLock()
        {
            if (autoLockMinutes > 0 && !isLocked && wallet != null)
            {
                ...
                if (inactiveMinutes >= autoLockMinutes)
                {
                    ...
                    if (showWalletInfo)
                    {
                        Debug.Log($"[EpicChainWalletComponent] Auto-locked wallet after {inactiveMinutes:F1} minutes of inactivity");
                    }
                }
            }
        }
        ...
        private void ShowWalletInfo()
        {
            if (wallet == null)
            {
                Debug.Log("[EpicChainWalletComponent] No wallet loaded");
                return;
            }
            
            Debug.Log($"[EpicChainWalletComponent] Wallet Info:\n" +
                     $"Name: {wallet.Name}\n" +
        ...
        private void ShowAccountAddresses()
        {
            ...
            Debug.Log($"[EpicChainWalletComponent] Account Addresses ({wallet.AccountCount}):");
