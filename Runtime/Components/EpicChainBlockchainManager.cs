...
namespace EpicChain.Unity.SDK.Components
{
    ...
    public class NeoBlockchainManager : MonoBehaviour
    {
        ...
        private EpicChainWallet currentWallet;
        ...
        public async Task<bool> InitializeBlockchain()
        {
            if (isInitialized)
            {
                Debug.LogWarning("[EpicChainBlockchainManager] Already initialized");
                return true;
            }
            ...
            try
            {
                ...
                // Initialize EpicChain Unity SDK
                var success = await EpicChainUnityInstance.Initialize(config);
                
                if (!success)
                {
                    throw new Exception("Failed to initialize EpicChain Unity SDK");
                }
                ...
                {
                    Debug.Log($"[EpicChainBlockchainManager] Successfully initialized with node: {config.NodeUrl}");
                }
            ...
            catch (Exception ex)
            {
                ...
                Debug.LogError($"[EpicChainBlockchainManager] {errorMessage}");
        ...
        public async Task<EpicChainWallet> CreateNewWallet()
        {
            try
            {
                currentWallet = await EpicChainWallet.Create();
                currentWallet.Name = defaultWalletName;
                
                if (enableEventLogging)
                {
                    Debug.Log($"[EpicChainBlockchainManager] Created new wallet: {currentWallet.DefaultAccount.Address}");
                }
            ...
            {
                Debug.LogError($"[EpicChainBlockchainManager] Failed to create wallet: {ex.Message}");
                throw;
            }
        }
        
        /// <summary>
        /// Loads a wallet from a XEP-6 file.
        /// </summary>
        /// <param name="filePath">Path to the XEP-6 wallet file</param>
        /// <returns>The loaded wallet</returns>
        public async Task<EpicChainWallet> LoadWallet(string filePath)
        {
            try
            {
                currentWallet = await EpicChainWallet.FromXEP6WalletFile(filePath);
                
                if (enableEventLogging)
                {
                    Debug.Log($"[EpicChainBlockchainManager] Loaded wallet: {currentWallet.Name} with {currentWallet.AccountCount} accounts");
                }
            ...
            {
                Debug.LogError($"[EpicChainBlockchainManager] Failed to load wallet: {ex.Message}");
                throw;
            }
        }
        ...
        public void SetCurrentWallet(EpicChainWallet wallet)
        {
            currentWallet = wallet;
            
            if (enableEventLogging && wallet != null)
            {
                Debug.Log($"[EpicChainBlockchainManager] Set current wallet: {wallet.Name}");
            }
        }
        ...
        private System.Collections.IEnumerator MonitorBlockchain()
        {
            while (isInitialized && Application.isPlaying)
            {
                ...
                catch (Exception ex)
                {
                    if (enableEventLogging)
                    {
                        Debug.LogWarning($"[EpicChainBlockchainManager] Monitoring error: {ex.Message}");
                    }
                }
        ...
        private async Task CheckForNewBlocks()
        {
            try
            {
                var response = await EpicChainUnityInstance.GetBlockCount().SendAsync();
                ...
                if (currentHeight > lastKnownBlockHeight)
                {
                    ...
                    {
                        Debug.Log($"[EpicChainBlockchainManager] New block detected: {currentHeight}");
                    }
            ...
            catch (Exception ex)
            {
                if (enableEventLogging)
                {
                    Debug.LogWarning($"[EpicChainBlockchainManager] Failed to check block height: {ex.Message}");
                }
            }
        }
        ...
        private async Task CheckForBalanceChanges()
        {
            try
            {
                var balances = await currentWallet.GetXep17TokenBalances();
                ...
                foreach (var kvp in balances)
                {
                    ...
                    if (lastKnownBalances.TryGetValue(tokenHash, out var lastBalance))
                    {
                        if (currentBalance != lastBalance)
                        {
                            if (enableEventLogging)
                            {
                                Debug.Log($"[EpicChainBlockchainManager] Balance changed for {tokenHash}: {lastBalance} → {currentBalance}");
                            }
            ...
            catch (Exception ex)
            {
                if (enableEventLogging)
                {
                    Debug.LogWarning($"[EpicChainBlockchainManager] Failed to check balances: {ex.Message}");
                }
            }
        }
        ...
        private async void CheckStatus()
        {
            if (!isInitialized)
            {
                Debug.Log("[EpicChainBlockchainManager] Status: Not Initialized");
                return;
            }
            ...
            {
                Debug.LogError($"[EpicChainBlockchainManager] Status check failed: {ex.Message}");
            }
        }
