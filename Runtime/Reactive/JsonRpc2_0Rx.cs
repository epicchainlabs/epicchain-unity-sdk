    ...
    public class JsonRpc2_0Rx
    {
            ...
            [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            while (!token.IsCancellationRequested)
            {
                try
                {
                    var response = await EpicChainUnityGetBlockCount();
            ...
            [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            await foreach (var blockIndex in GetBlockIndexStream(pollingIntervalMs, token))
            {
                try
                {
                    var response = await EpicChainUnityGetBlock(blockIndex, fullTransactionObjects);
            ...
            [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var blockCountResponse = await EpicChainUnityGetBlockCount();
