...
namespace EpicChain.Unity.SDK.Contracts.Native
{
    ...
    public class NeoNameService : NonFungibleToken
    {
        ...
        #region XEP-11 Overrides for Domain Names
        ...
        public async Task<TransactionBuilder> AddRoot(string rootDomain)
        {
            ...
            {
                Debug.Log($"[EpicChainNameService] Adding root domain: {rootDomain}");
            }
        ...
        public async Task<List<string>> GetRootsUnwrapped(int count = DEFAULT_ITERATOR_COUNT)
        {
            ...
            if (EpicChainUnityConfig.EnableDebugLogging)
            {
                Debug.Log($"[EpicChainNameService] Retrieved {roots.Count} root domains");
            }
        ...
        public async Task<bool> IsAvailable(string domainName)
        {
            ...
            if (EpicChainUnityConfig.EnableDebugLogging)
            {
                Debug.Log($"[EpicChainNameService] Domain '{domainName}' availability: {result}");
            }
        ...
        public async Task<TransactionBuilder> Register(string domainName, Hash160 owner)
        {
            ...
            if (EpicChainUnityConfig.EnableDebugLogging)
            {
                Debug.Log($"[EpicChainNameService] Registering domain '{domainName}' for owner: {owner}");
            }
        ...
        public async Task<string> GetRecord(string domainName, RecordType recordType)
        {
            ...
            try
            {
                ...
                if (EpicChainUnityConfig.EnableDebugLogging)
                {
                    Debug.Log($"[EpicChainNameService] Record {recordType} for '{domainName}': {result}");
                }
        ...
        public async Task<string> Resolve(string domainName, RecordType recordType)
        {
            ...
            try
            {
                ...
                if (EpicChainUnityConfig.EnableDebugLogging)
                {
                    Debug.Log($"[EpicChainNameService] Resolved '{domainName}' ({recordType}): {result}");
                }
