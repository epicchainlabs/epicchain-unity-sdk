...
namespace EpicChain.Unity.SDK.Contracts
{
    ...
    public class Iterator<T> : IDisposable
    {
        ...
        public async Task<List<T>> Traverse(int count = 100)
        {
            ...
            try
            {
                var response = await EpicChainUnityTraverseIterator(sessionId, iteratorId, count).SendAsync();
        ...
        public async Task<bool> TerminateSession()
        {
            ...
            try
            {
                var response = await EpicChainUnityTerminateSession(sessionId).SendAsync();
