...
namespace EpicChain.Unity.SDK.Transaction
{
    ...
    public class TransactionBuilder
    {
        ...
        /// <summary>XEP-17 balanceOf function name</summary>
        ...
        public async Task<NeoTransaction> GetUnsignedTransaction()
        {
            ...
            {
                var currentBlockCount = await EpicChainUnityGetBlockCount().SendAsync();
                var blockCount = currentBlockCount.GetResult();
                validUntilBlock = (uint)(blockCount + EpicChainUnityMaxValidUntilBlockIncrement - 1);
            }
        ...
        public async Task<NeoInvokeScriptResponse> CallInvokeScript()
        {
            ...
            return await EpicChainUnityInvokeScript(script.ToHexString(), signers).SendAsync();
        }
        ...
        private async Task<bool> IsAllowedForHighPriority()
        {
            try
            {
                var committeeResponse = await EpicChainUnityGetCommittee().SendAsync();
        ...
        private async Task<long> GetSystemFeeForScript()
        {
            try
            {
                var response = await EpicChainUnityInvokeScript(script.ToHexString(), signers).SendAsync();
                var result = response.GetResult();
                
                if (result.HasStateFault() && !EpicChainUnityConfig.AllowTransmissionOnFault)
                {
        ...
        private async Task<long> CalculateNetworkFee()
        {
            try
            {
                ...
                var response = await EpicChainUnityCalculateNetworkFee(tempTransaction.ToArray().ToHexString()).SendAsync();
        ...
        private async Task<long> GetSenderGasBalance()
        {
            try
            {
                var response = await EpicChainUnityInvokeFunction(
                    GAS_TOKEN_HASH,
                    BALANCE_OF_FUNCTION,
