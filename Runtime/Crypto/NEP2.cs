...
namespace EpicChain.Unity.SDK.Crypto
{
    ...
    public static class XEP2
    {
        ...
        private const int XEP2_PREFIX_1 = 0x01;
        private const int XEP2_PREFIX_2 = 0x42;
        private const int XEP2_FLAGBYTE = 0xE0;
        ...
        /// <summary>
        /// Encrypts a private key using XEP-2 standard with password protection.
        /// </summary>
        ...
        /// <returns>XEP-2 encrypted private key string</returns>
        public static async Task<string> Encrypt(string password, ECKeyPair keyPair, ScryptParams scryptParams = null)
        {
            ...
            try
            {
                ...
                using (var aes = Aes.Create())
                {
                    ...
                    using (var encryptor = aes.CreateEncryptor())
                    {
                        ...
                        // Build XEP-2 format: prefix + flag + addressHash + encrypted
                        var result = new byte[ENCRYPTED_KEY_LENGTH];
                        result[0] = XEP2_PREFIX_1;
                        result[1] = XEP2_PREFIX_2;
                        result[2] = XEP2_FLAGBYTE;
            ...
            {
                throw new CryptographicException($\"XEP-2 encryption failed: {ex.Message}\", ex);
            }
        }
        
        /// <summary>
        /// Decrypts a XEP-2 encrypted private key using the provided password.
        /// </summary>
        /// <param name="password">The password to decrypt with</param>
        /// <param name="encryptedKey">The XEP-2 encrypted private key</param>
        /// <param name="scryptParams">Scrypt parameters for key derivation</param>
        ...
        public static async Task<ECKeyPair> Decrypt(string password, string encryptedKey, ScryptParams scryptParams = null)
        {
            ...
            try
            {
                // Decode the XEP-2 key
                var decoded = encryptedKey.Base58CheckDecoded();
                if (decoded == null || decoded.Length != ENCRYPTED_KEY_LENGTH)
                    throw new ArgumentException("Invalid XEP-2 encrypted key format.");
                
                // Validate XEP-2 format
                if (decoded[0] != XEP2_PREFIX_1 || decoded[1] != XEP2_PREFIX_2 || decoded[2] != XEP2_FLAGBYTE)
                    throw new ArgumentException("Invalid XEP-2 key format.");
            ...
            {
                throw new CryptographicException($"XEP-2 decryption failed: {ex.Message}", ex);
            }
        }
        ...
        /// <summary>
        /// Simplified scrypt implementation for XEP-2 key derivation.
        /// </summary>
        /// <param name="password">Password bytes</param>
        /// <param name="salt">Salt bytes</param>
        ...
        /// <summary>
        /// Validates if a string is a valid XEP-2 encrypted key format.
        /// </summary>
        /// <param name="encryptedKey">The encrypted key to validate</param>
        /// <returns>True if valid XEP-2 format</returns>
        public static bool IsValidXEP2Key(string encryptedKey)
        {
            if (string.IsNullOrEmpty(encryptedKey))
                return false;
    ...
    /// <summary>
    /// Scrypt parameters for XEP-2 key derivation.
    /// </summary>
    [System.Serializable]
    public class ScryptParams
    {
        ...
        /// <summary>Default XEP-2 scrypt parameters (N=16384, r=8, p=8)</summary>
