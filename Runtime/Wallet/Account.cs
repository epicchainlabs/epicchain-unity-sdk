...
using EpicChain.Unity.SDK.Script;
...
namespace EpicChain.Unity.SDK.Wallet
{
    ...
    public class Account
    {
        ...
        private EpicChainWallet wallet;
        ...
        internal Account(ECKeyPair keyPair, string address, string label, VerificationScript verificationScript,
                        bool isLocked, string encryptedPrivateKey, EpicChainWallet wallet, 
        ...
        /// <returns>This account for method chaining</returns>
        internal Account SetWallet(EpicChainWallet wallet)
        {
            this.wallet = wallet;
            return this;
        }
        ...
        /// <summary>
        /// Decrypts this account's private key according to the XEP-2 standard, if not already decrypted.
        /// </summary>
        ...
        public async Task DecryptPrivateKey(string password, ScryptParams scryptParams = null)
        {
            ...
            try
            {
                ...
                keyPair = await XEP2.Decrypt(password, encryptedPrivateKey, scryptParams);
        ...
        /// <summary>
        /// Encrypts this account's private key according to the XEP-2 standard.
        /// </summary>
        ...
        public async Task EncryptPrivateKey(string password, ScryptParams scryptParams = null)
        {
            ...
            {
                scryptParams = scryptParams ?? ScryptParams.Default;
                encryptedPrivateKey = await XEP2.Encrypt(password, keyPair, scryptParams);
        ...
        public async Task<Dictionary<Hash160, long>> GetXep17Balances(EpicChainUnity epicchainUnity = null)
        {
            ...
            try
            {
                var response = await EpicChainUnityGetXep17Balances(GetScriptHash()).SendAsync();
            ...
            {
                throw new WalletException($"Failed to get XEP-17 balances for account {address}: {ex.Message}", ex);
            }
        }
        ...
        #region XEP-6 Conversion
        
        /// <summary>
        /// Converts this account to a XEP-6 account format.
        /// </summary>
        /// <returns>The XEP-6 account representation</returns>
        public async Task<XEP6Account> ToXEP6Account()
        {
            ...
            XEP6Contract contract = null;
            
            if (verificationScript != null)
            {
                var parameters = new List<XEP6Parameter>();
                ...
                if (verificationScript.IsMultiSigScript())
                {
                    ...
                    {
                        parameters.Add(new XEP6Parameter($"signature{i}", ContractParameterType.Signature));
                    }
                }
                else if (verificationScript.IsSingleSigScript())
                {
                    parameters.Add(new XEP6Parameter("signature", ContractParameterType.Signature));
                }
                
                var scriptBase64 = Convert.ToBase64String(verificationScript.Script);
                contract = new XEP6Contract(scriptBase64, parameters, false);
            }
            
            return new XEP6Account(address, label, IsDefault, isLocked, encryptedPrivateKey, contract, null);
        }
        ...
        /// <summary>
        /// Creates an account from a XEP-6 account.
        /// </summary>
        /// <param name="xep6Account">The XEP-6 account</param>
        /// <returns>The account</returns>
        public static async Task<Account> FromXEP6Account(XEP6Account xep6Account)
        {
            if (xep6Account == null)
                throw new ArgumentNullException(nameof(xep6Account));
            
            VerificationScript verificationScript = null;
            int? signingThreshold = null;
            int? numberOfParticipants = null;
            
            if (xep6Account.Contract?.Script != null && !string.IsNullOrEmpty(xep6Account.Contract.Script))
            {
                var scriptBytes = Convert.FromBase64String(xep6Account.Contract.Script);
            ...
            return new Account(null, xep6Account.Address, xep6Account.Label, verificationScript,
                              xep6Account.Lock, xep6Account.Key, null, signingThreshold, numberOfParticipants);
        }
